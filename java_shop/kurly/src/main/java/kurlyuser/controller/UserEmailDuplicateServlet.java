...
public class UserEmailDuplicateServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            boolean result = userDao.emailCheckMethod(user.getUserEmail());

            if (result) {
                response.setStatus(HttpServletResponse.SC_CONFLICT);
                response.getWriter().write("{\"result\": "+ result+", \"reason\": \"Email이 존재합니다.\"}");
            } else {
